...
void Expression::checkUsed(AnalysisResultPtr ar,
                           ExpressionPtr variable, ExpressionPtr value) {
  // if the value may involve object, consider the variable as "used"
  // so that objects are not destructed prematurely.
  bool used = true;
  TypePtr type = value ? value->getType() : TypePtr();
  if (value && value->isScalar()) used = false;
  if (type && type->isNoObjectInvolved()) used = false;
  if (used && variable->is(Expression::KindOfSimpleVariable)) {
