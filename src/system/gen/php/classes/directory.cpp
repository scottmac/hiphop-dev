...
void c_directory::o_get(Array &props) const {
  if (isInitialized(m_path)) props.set("path", m_path.isReferenced() ? ref(m_path) : m_path);
  if (isInitialized(m_handle)) props.set("handle", m_handle.isReferenced() ? ref(m_handle) : m_handle);
