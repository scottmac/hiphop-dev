...
static void php_array_merge_recursive(Array &arr1, CArrRef arr2) {
  if (arr2->supportValueRef()) {
    for (ArrayIter iter(arr2); iter; ++iter) {
      ...
      if (key.isNumeric()) {
        ...
        php_array_merge_recursive(subarr1, value.toArray());
...
Variant f_array_merge_recursive(int _argc, CVarRef arr1, CArrRef args) {
  ...
  Array ret = Array::Create();
  php_array_merge_recursive(ret, arr1.toArray());
  for (ArrayIter iter(args); iter; ++iter) {
    Variant v = iter.second();
    if (!v.isArray()) {
      throw_bad_array_exception("f_array_merge_recursive");
    ...
    php_array_merge_recursive(ret, v.toArray());
  }
  return ret;
}
...
static void php_array_replace_recursive(Array &arr1, CArrRef arr2) {
  if (arr2->supportValueRef()) {
    for (ArrayIter iter(arr2); iter; ++iter) {
      Variant key = iter.first();
      CVarRef value = iter.secondRef();
      if (arr1.exists(key) && arr1[key].isArray() && value.isArray()) {
        Array subarr1 = arr1[key].toArray();
        php_array_replace_recursive(subarr1, value.toArray());
        arr1.set(key, subarr1);
      } else {
        if (value.isReferenced()) value.setContagious();
        arr1.set(key, value);
...
Variant f_array_replace_recursive(int _argc, CVarRef array1,
  ...
  Array ret = Array::Create();
  php_array_replace_recursive(ret, array1);
  for (ArrayIter iter(_argv); iter; ++iter) {
    Variant v = iter.second();
    if (!v.isArray()) {
      throw_bad_array_exception("f_array_replace_recursive");
    ...
    php_array_replace_recursive(ret, v.toArray());
  }
  return ret;
}
