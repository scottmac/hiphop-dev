...
void c_arrayiterator::o_get(Array &props) const {
  if (isInitialized(m_arr)) props.set(String("\0arrayiterator\0arr", 18, CopyString), m_arr.isReferenced() ? ref(m_arr) : m_arr);
  if (isInitialized(m_flags)) props.set(String("\0arrayiterator\0flags", 20, CopyString), m_flags.isReferenced() ? ref(m_flags) : m_flags);
...
void c_appenditerator::o_get(Array &props) const {
  if (isInitialized(m_iterators)) props.set(String("\0appenditerator\0iterators", 25, CopyString), m_iterators.isReferenced() ? ref(m_iterators) : m_iterators);
  c_ObjectData::o_get(props);
}
