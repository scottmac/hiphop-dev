...
void c_exception::o_get(Array &props) const {
  if (isInitialized(m_message)) props.set("message", m_message.isReferenced() ? ref(m_message) : m_message);
  if (isInitialized(m_code)) props.set("code", m_code.isReferenced() ? ref(m_code) : m_code);
  if (isInitialized(m_file)) props.set("file", m_file.isReferenced() ? ref(m_file) : m_file);
  if (isInitialized(m_line)) props.set("line", m_line.isReferenced() ? ref(m_line) : m_line);
...
void c_errorexception::o_get(Array &props) const {
  if (isInitialized(m_severity)) props.set("severity", m_severity.isReferenced() ? ref(m_severity) : m_severity);
  c_exception::o_get(props);
}
